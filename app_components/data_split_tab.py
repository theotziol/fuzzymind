...
def split_input_target():
    ...
    if len(st.session_state.output_columns) == 0:
        ...
        if st.session_state.learning_task == 'Regression': 
            ...
            with cl2:
                st.radio('Select the regression splitting method', ['Standard split', 'Timestep split'], 1, key = 'regression_split', help = help_regression_split)
                if st.session_state.regression_split == 'Timestep split':
                    st.slider('Select timestep number', 1, 8, 1, 1, key = 'timestep_num', help = help_timestep)
                    submit = st.button('Submit', on_click = submit_splitting_parameters_timestep_callback)
                elif st.session_state.regression_split == 'Standard split':
            ...
            if submit:
                ...
                with col2:
                    ...
                    if st.session_state.regression_split == 'Timestep split':
                        st.dataframe(st.session_state.output_df[[i for i in st.session_state.output_df.columns if i not in st.session_state.input_df.columns]])
                    else:
...
def submit_splitting_parameters_callback():
    ...
    st.session_state.input_df, st.session_state.output_df = _split_labels(st.session_state.working_df, st.session_state.output_columns, value = st.session_state.default_output_value, shuffle=shuffle)


def submit_splitting_parameters_timestep_callback():
    st.session_state.initialized_preprocessing = True
    
    # st.session_state.input_df = st.session_state.working_df[[i for i in st.session_state.working_df.columns if i not in st.session_state.output_columns]]
    # st.session_state.output_df = st.session_state.working_df[st.session_state.output_columns]
    st.session_state.input_df, st.session_state.output_df = _split_input_target_timeseries(st.session_state.working_df, timesteps= st.session_state.timestep_num)
            
        
### Preprocessing methods. 
## To-do: Consider mobing them in a preprocessing script
