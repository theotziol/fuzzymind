...
help_offset = (
    "Weight matrices are initialized randomly with values [-1 + x_offset, 1x-_offset]."
)
...
def learning_method_widgets():
    ...
    with col2:
        st.radio(
            ...
            ["Neural-FCM", "Particle Swarm Optimization"],
            0,
            help=help_learning_algorithms,
    ...
    if st.session_state.learning_algorithm is not None:
        st.write(
            f"You selected {st.session_state.learning_algorithm} for {st.session_state.learning_task}"
        )
    ...
    with st.expander(
        ...
        with col2:
            ...
            st.checkbox(
                "Early stopping",
                True,
                key="bool_early_stopping",
                help=help_early_stopping,
            )
            if st.session_state.bool_early_stopping:
                ...
                with cl1:
                    st.slider(
                        ...
                        1,
                        key="patience",
                        help="The number of epochs that the early stopping algorithm shall wait before stopping the training.",
                    )
                with cl2:
                    st.write("")
                    st.checkbox(
                        ...
                        True,
                        key="restore_best_weights",
                        help="Whether to acquire the weights of the epoch where the minimum validation error was occured or at the final epoch (after patience epochs)",
                    )
            st.slider(
                "Learning rate",
                0.0001,
                ...
                format="%0.4f",
                key="learning_rate",
                help="The learning rate for the [Adam](https://keras.io/api/optimizers/adam/) optimizer. Recommended 0.001, (Adam default)",
            )
...
def parameters_tab_pso():
    ...
    with st.expander(
        "PSO-FCM parameters...", expanded=not st.session_state.training_finished
    ):
...
def results_widgets():
    ...
    if st.session_state.training_finished:
        ...
        if st.session_state.split_method == "KFold":
            ...
            with cl2:
                select_fold = st.selectbox(
                    "Select fold results...",
                    st.session_state.kfold_dic.keys(),
                    None,
                    format_func=lambda option: f"Fold {option}",
                    placeholder="Choose a fold",
                )
            if select_fold != None:
                st.caption(f"Fold {select_fold} results.")
                tab_learning_results, tab_testing_results, tab_matrix = st.tabs(
                    ["🎓 Learning Results", "📑 Testing Results", "🧮 Weight Matrix"]
                )
...
def learning_neuralfcm_classification_standard():
    with st.spinner("Learning has started. This may take a while..."):
        ...
        y_train = st.session_state.output_df.iloc[
        ...
        nfcm = neural_fcm(
            x_train.shape[-1],
            y_train.shape[-1],
            fcm_iter=st.session_state.fcm_iter,
            l_slope=st.session_state.l_slope,
        )
...
def learning_neuralfcm_classification_KFold():
    ...
    with st.status(
        ...
        status.update(
            label=f"{st.session_state.kfold_n_splits}Fold has finished!",
            state="complete",
            expanded=False,
        )
...
def learning_neuralfcm_regression_standard():
    with st.spinner("Learning has started. This may take a while..."):
        train_x, test_x, train_y, test_y = split_train_test(
            st.session_state.input_df.to_numpy(),
            st.session_state.output_df.to_numpy(),
        ...
        if st.session_state.timestep_num != None:
            nfcm = neural_fcm(
                train_x.shape[-1],
                output_concepts=len(st.session_state.output_columns),
                fcm_iter=st.session_state.fcm_iter,
                l_slope=st.session_state.l_slope,
            )
            ...
            nfcm.initialize_loss_and_compile(
                "mse_standard", lr=st.session_state.learning_rate
            )
            history = nfcm.model.fit(
                train_x,
                y_train,
    ...
    if st.session_state.training_finished:
        ...
        with st.status("Testing on unseen data...", expanded=True) as status:
            ...
            finish = time.time()
            real_array_test = (
                st.session_state.non_norm_working_df[st.session_state.output_columns]
                .iloc[-len(test_y) :]
                .to_numpy()
            )
            real_array = st.session_state.non_norm_working_df[
                st.session_state.output_columns
            ].to_numpy()
            nfcm.statistics_regression(real_array_test[:, 0], real_array)
            nfcm.statistics_regression_norm(
                test_y[:, -len(st.session_state.output_columns)]
            )
            st.session_state.model = nfcm
            st.session_state.model.prediction_time = np.round(finish - start, 4)
            status.update(label="Testing Completed", state="complete", expanded=False)
...
def learning_neuralfcm_regression_KFold():
    ...
    with st.status(
        ...
        for train_index, test_index in kf.split(st.session_state.input_df):
            ...
            nfcm.statistics_regression(real_array_test[:, 0], real_array)
            nfcm.statistics_regression_norm(
                y_test[:, -len(st.session_state.output_columns)]
            )
