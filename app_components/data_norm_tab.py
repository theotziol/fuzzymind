...
standard_scaler_formula = (
    "$x_i^{new} = \\dfrac{x_i^{old} - \\textbf{x}_{mean}}{\\textbf{x}_{std}}$"
)
...
def data_normalization():
    ...
    if len(text_columns) > 0:
        st.warning(
            f"The working dataset contains the following non-numeric columns: \n{text_columns}",
            icon="⚠️",
        )
        st.info(
            "Use the **Data Transformation** tab to modify these columns, or the **Dataset Parameters** tab to delete columns."
        )
    else:
        ...
        with col1:
            ...
            method = st.selectbox(
                "Select the normalization method", ["Min-Max Normalization"], 0
            )
        ...
        with col3:
            if method == "Min-Max Normalization":
                ...
                st.write(
                    "The z-score method (often called standardization) transforms each column into distribution with a mean of 0 and a typical deviation of 1. The following formula is applied:"
                )
        ...
        if method == "Min-Max Normalization":
            ...
            st.warning(
                "This method is not compatible with the FCM theory that expects FCM concept values to be in [0, 1]. Please select the Min-Max method for consistency!"
            )
            apply_changes = st.button(
                "Apply normalization", on_click=apply_standard_callback, disabled=True
            )
            if apply_changes:
                st.success("Dataset was succesfully normalized.")
...
def check_text(df):
    ...
    columns = [
        ...
        if st.session_state.working_df[col].dtype.name in text_dtypes
...
def apply_min_max_callback():
    ...
    for column in st.session_state.working_df.columns:
        st.session_state.working_df[column] = (
            st.session_state.working_df[column]
...
def apply_standard_callback():
    for column in st.session_state.working_df.columns:
        st.session_state.working_df[column] = (
            st.session_state.working_df[column]
            - st.session_state.working_df[column].mean()
        ) / (st.session_state.working_df[column].std())
    st.session_state.changed = True
