...
def plot_widgets():
    ...
    columns = [
        ...
        if st.session_state.working_df[col].dtype.name not in not_plotting
    ]

    chart_type = st.selectbox(
        "Select chart type", ["Line", "Area", "Bar", "Boxplot", "Histogram"]
    )
...
def plot_column(column, chart_type):
    try:
        ...
        if chart_type == "Line":
            ...
            axs.axvline(
                st.session_state.working_df[column].median(),
                color="k",
                linestyle="dashed",
                linewidth=1,
                label="median",
